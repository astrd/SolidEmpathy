...
//File explorer methods here

export const getPublicFolderRoot = async () =>{
  let myId = await getWebId();
  console.log(myId);
  let parts = myId.split("//");
  parts = parts[1].split("/");
  let publicRoot = "https://" + parts[0] + '/public/';
  return publicRoot;
}

export const isContainer = async (resource) => {
  let types = await getFieldArray(resource, "type", RDFN);
  var container = false;
  for(var i = 0; i < types.length; i++){
    if(types[i].value === LDP("Container").value){
      container = true;
    }
  }
  return container;
...
export const getFolderContent = async (folderUrl) =>{
  let content = await getFieldArray(folderUrl, "contains", LDP);
  let clasifiedContent = {
    folders: [],
    archives: []
  }
  for(var i = 0; i< content.length; i++){
    let container = await isContainer(content[i]);
    if(container){
      clasifiedContent.folders.push(await getResourceData(content[i]));
    }else{
      clasifiedContent.archives.push(await getResourceData(content[i]));
    }
  }
...
export const getResourceData = async (resource) =>{
  let splitName = resource.value.split("//");
  let n = splitName[splitName.length-1].split("/");
  n = n[n.length-2];
  let data =  {
    name: n,
    url: resource.value,
    modified: await getField(resource, "modified", TERMS),
    size: await getField(resource, "size", STATS)
  }
  console.log(data);
  return data;
}
